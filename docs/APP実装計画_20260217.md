...
## 8. 追加実装（2026-02-17 18:05時点）

- `GET /api/v1/invoices` / `POST /api/v1/invoices` を実装
- `GET /api/v1/payments` / `POST /api/v1/payments` を実装
- Excel同期に `支払管理` 取り込みを追加（`payments_upserted`）
- Web操作パネルに以下を追加
  - 請求登録パネル
  - 支払登録パネル
  - 請求/支払一覧パネル
